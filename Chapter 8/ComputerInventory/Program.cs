    ...
    public class Program {
        ...
        static int MainMenu() {
            ...
            do {
                ...
                Console.WriteLine("6. Support Tickets and Log");
                ...
                try {
                    ...
                    if (result == 1) {
                        ...
                        using (MachineContext context = new MachineContext()) {
                            ...
                            var machineIdParam = new SqlParameter("@MachineId", 10);
        ...
        public static bool CheckForExistingOS(string osName) {
            bool exists = false;
        ...
        static void UpdateOperatingSystems() {
            ...
            using (MachineContext context = new MachineContext()) {
                foreach (Machine m in lMachines) {
                    var mach = context.Machine.Single(x => x.MachineId == m.MachineId);
                    mach.OperatingSysId = newOsId;
                    context.Update(mach);
                    try {
                        context.SaveChanges();
                    }
                        ...
                        foreach (var e in ex.Entries) {
                            if (e.Entity is Machine) {
                                var dbEntity = context.Machine.AsNoTracking().Single(m1 => m1.MachineId == ((Machine)e.Entity).MachineId);
                                ...
                                foreach (var property in e.Metadata.GetProperties()) {
                                    if (property.Name == "OperatingSysId") {
                                        ...
                                        Console.WriteLine("The value has been changed outside of the program.");
                                        OperatingSys osN = GetOperatingSystemById(Convert.ToInt32(dbValue));
                                        Console.WriteLine($"The new value is {osN.Name}");
                                        osN = GetOperatingSystemById(Convert.ToInt32(proposedValue));
                                        Console.WriteLine($"You entered {osN.Name}");
                                        Console.WriteLine("Do you want to overwrite this change? [y or n]");
                                        cki = Console.ReadKey(true);
                                        result = cki.KeyChar.ToString();
                                        if (result.ToLower() == "y") {
                                            e.Property(property.Name).OriginalValue = dbEntry.Property(property.Name).CurrentValue;
                                            //try again
                                            context.SaveChanges();
                                        }
                                    }
        ...
        static void LogicalOr() {
            using (var context = new MachineContext()) {
                var machines = from m in context.Machine
                               ...
                               where (m.OperatingSysId == 22 || m.OperatingSysId == 24)
                               orderby m.OperatingSysId
                               select new { m.Name, osName = m.OperatingSys.Name };

                foreach (var m in machines) {
                    Console.WriteLine($"{m.Name} is running {m.osName}");
                }
            }
        }
            ...
            do {
                ...
                try {
                    ...
                    if (result == 1) {
                        ...
                        SupportTicketLogEntry oSupport = new SupportTicketLogEntry();
                        oSupport.UpdateSupportTicket();
                    }
                    else if (result == 3) {
                        SupportTicketLogEntry oSupport = new SupportTicketLogEntry();
                        oSupport.ViewSupportTickets();
                    }
                    else if (result == 9) {
                        // We are exiting so nothing to do
                        cont = true;
